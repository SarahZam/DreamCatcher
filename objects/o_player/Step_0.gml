...
//horizontal
if (place_meeting(x+hsp,y,o_wall) || place_meeting(x+hsp,y,o_jungle_boss))
{
	while(!place_meeting(x+sign(hsp),y,o_wall) && !place_meeting(x+sign(hsp),y,o_jungle_boss))
	{
		x = x + sign(hsp);
	}
	hsp = 0;
...
//collision check
if ((place_meeting(x,y+vsp,o_wall) || place_meeting(x,y+vsp,o_jungle_boss)) && !(key_jump && (place_meeting(x,y+vsp,o_ladder_1) || place_meeting(x,y+vsp,o_ladder_2))))
{	while(!place_meeting(x,y+sign(vsp),o_wall) && !place_meeting(x,y+sign(vsp),o_jungle_boss))
	{
		y = y + sign(vsp);
	}	
	vsp = 0;
